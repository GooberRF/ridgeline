...
static intptr_t get_exported_fun_offset_from_module_base(const char* dll_filename, const char* fun_name)
{
    ...
    if (!proc_local_ptr) {
        // MSVC + __stdcall + __declspec(dllexport) on x86 decorates the
        // export name as "_<name>@<argbytes>". Module DLLs declare Init as
        // `DWORD WINAPI Init(LPVOID)`, so the decorated form is "_Init@4".
        // Try that fallback before giving up — saves every module from
        // needing to ship a .def file just to undecorate the export.
        std::string decorated = std::string{"_"} + fun_name + "@4";
        proc_local_ptr = GetProcAddress(local_lib, decorated.c_str());
        if (proc_local_ptr) {
            xlog::info("Resolved {} via stdcall-decorated name {}", fun_name, decorated);
        }
    }
