                           ...
                           const std::string& log_path)
{
    ...
    GetModuleFileNameA(self ? self : GetModuleHandleA(nullptr),
    ...
    cfg.this_module_handle = self ? self : GetModuleHandleA(nullptr);
    std::strncpy(cfg.output_dir, logs_dir.c_str(),  sizeof(cfg.output_dir) - 1);
    std::strncpy(cfg.log_file,   log_path.c_str(),  sizeof(cfg.log_file)   - 1);
    std::strncpy(cfg.app_name,   app_name.c_str(),  sizeof(cfg.app_name)   - 1);

    CrashHandlerStubInstall(cfg);
}
